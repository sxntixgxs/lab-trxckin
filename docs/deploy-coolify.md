...
Coolify lists each variable twice, with the types **Production** and **Preview**. Deploys only read the Production ones. Paste each value alone: Coolify keeps a value such as `WORKOS_API_KEY=sk_…` as is, and WorkOS rejects it. The frontend logs a `[config]` error at startup when a WorkOS value looks like that.
...
**Sign-in and sign-up both end on "Sign-in didn't finish"**
WorkOS refused the code exchange. The frontend's logs have an `[auth] sign-in failed` line with its reason. `invalid_client` ("Invalid client secret") means `WORKOS_API_KEY` doesn't belong to the WorkOS environment of `WORKOS_CLIENT_ID`, or it was pasted with its name, quotes or spaces; a `[config]` line at startup points at the latter. Use one environment's client ID and API key in Coolify's Production variables and in Convex, then Coolify → Redeploy.
