...
import { renderFormularioClientePdfBlob } from "@/app/(default)/customers/onboarding/pdf/formulario-pdf-cliente-data";
...
const SUB_VISTA = "Vista del formulario de inscripción del cliente.";

const FASES_POSTERIORES: ReadonlySet<string> = new Set(["III_REVISION_DOCUMENTAL", "IIIA_APROBACION_CUMPLIMIENTO", "IV_CREACION_CONTABILIDAD", "COMPLETADO"]);
...
function SignContent() {
  ...
  const firmar = useMutation(api.onboarding.customersPublic.firmarFormularioRepresentante);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const blob = await renderFormularioClientePdfBlob(inscripcion);
        if (cancelled) return;
        setPdfUrl((prev) => {
  ...
  if (signed) {
    ...
    return (
      <PublicShell branding={branding} titulo={TITULO_FIRMA} subtitulo={SUB_FIRMA} center>
        ...
        >
          <p className="text-sm text-slate-600">El proceso de inscripción ha avanzado a la revisión documental. El equipo de Cumplimiento se pondrá en contacto si se requiere información adicional.</p>
        </StatusCard>
      </PublicShell>
    );
  }
  ...
  // Internal viewers (read-only token) and customers following their form link only see the PDF.
    ...
    return (
      <PublicShell branding={branding} titulo={TITULO_FIRMA} subtitulo={SUB_FIRMA} center>
        <StatusCard
          ...
          descripcion={esFirmada ? "Este formulario ya fue firmado. El proceso ha continuado a la siguiente etapa." : "El formulario aún no ha sido enviado por el cliente o el proceso cambió de estado."}
