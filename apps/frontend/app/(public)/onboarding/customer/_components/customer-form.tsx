...
import type { SupplierForm } from "../../supplier/_components/form-ui";
import { SeccionAccionaria } from "../../supplier/_components/section-accionaria";
import { DeclaracionesYEnvio, SeccionDocumentos, type DocUploadState } from "../../supplier/_components/section-documentos";
import { SeccionBancaria, SeccionContactos, SeccionReferencias } from "../../supplier/_components/sections-comerciales";
import { SeccionActividad, SeccionConflicto } from "../../supplier/_components/sections-generales";
import { buildSectionsPayload, collectErrorPaths, DEFAULT_FORM_VALUES, FIELD_SECTION, formSchema, STEPS, toFormValues, TRIBUTARIA_FIELD_LABELS, type FormValues, type InscripcionPublica } from "./form-schema";
import { SeccionAdicionales, SeccionDatosGenerales, SeccionTributaria } from "./sections-cliente";
...
/**
 * Fase II: the 10-section customer form with autosave, required documents and submit.
 * Sections that are identical to the supplier form (actividad, conflicto, accionaria,
 * contactos, bancaria, referencias) are shared; the form keeps their field keys and the
 * payload builder maps them to the customer schema names.
 */
export function CustomerForm({
  ...
  const actualizar = useMutation(api.onboarding.customersPublic.actualizarInscripcion);
  const enviarFormulario = useMutation(api.onboarding.customersPublic.enviarFormulario);
  ...
  // Shared sections are typed against the supplier form; the field paths they touch exist here too.
  const sharedForm = form as unknown as SupplierForm;
  ...
  async function handleDocUpload(docKey: string, file: File) {
    ...
    try {
      ...
      await actualizar({ ...identidad, documentos_09: { [docKey]: storageId } });
  ...
  const handleSubmit = form.handleSubmit(
    async (values) => {
      setSubmitError(null);
      try {
        const documentos_09: Record<string, Id<"_storage">> = {};
        for (const [key, v] of Object.entries(docUploads)) if (v.storageId) documentos_09[key] = v.storageId as Id<"_storage">;
        await actualizar({ ...identidad, ...buildSectionsPayload(values), documentos_09 });
  ...
  const subiendo = Object.values(docUploads).some((u) => u.uploading);
  ...
  return (
    <div className="flex min-h-screen flex-col bg-slate-50 text-slate-900" style={brandingVars(branding)}>
      <PublicHeader
        branding={branding}
        titulo={TITULO}
        subtitulo="Complete todos los campos requeridos y adjunte los documentos solicitados para iniciar su proceso de inscripción como cliente."
        autosave={autosave}
      />
          ...
          <div className="sticky top-20 w-56 shrink-0">
            ...
            {branding.contactEmail && (
              <div className="mt-6 rounded-xl border border-primary/20 bg-primary/5 px-4 py-4">
                ...
                <p className="mt-1 text-xs text-slate-500">Contacte a su asesor comercial ({branding.contactEmail}) para soporte con el formulario.</p>
              </div>
            )}
          </div>
        </aside>
        ...
        <main className="min-w-0 flex-1">
          ...
          <Form {...form}>
            <form onSubmit={handleSubmit} className="space-y-6">
              <SeccionDatosGenerales form={form} register={registerSection} branding={branding} tipoSolicitud={inscripcion.datos_generales_01.tipoSolicitud} />
              <SeccionActividad form={sharedForm} register={registerSection} branding={branding} />
              <SeccionConflicto form={sharedForm} register={registerSection} branding={branding} />
              <SeccionTributaria form={form} register={registerSection} branding={branding} />
              <SeccionAccionaria form={sharedForm} register={registerSection} />
              <SeccionContactos form={sharedForm} register={registerSection} />
              <SeccionBancaria form={sharedForm} register={registerSection} />
              <SeccionReferencias form={sharedForm} register={registerSection} />
              <SeccionAdicionales form={form} register={registerSection} condicionesPago={inscripcion.condicionesPago_12} />
              <SeccionDocumentos
                register={registerSection}
                index={9}
                step="10"
                tipoEvaluacion={inscripcion.tipoEvaluacion ?? "—"}
                ...
                onRemove={removeDoc}
              />
              <p className="text-xs text-slate-500">Los documentos que no cargue ahora podrá adjuntarlos después de firmar el formulario, durante la revisión documental.</p>
              <DeclaracionesYEnvio modulo="customer" branding={branding} submitting={form.formState.isSubmitting} disabled={subiendo} onSubmit={() => void handleSubmit()} />
            </form>
          </Form>
        </main>
      </div>
