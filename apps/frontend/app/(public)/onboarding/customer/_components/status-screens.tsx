...
/** Screens for every phase in which the customer has nothing to fill (I, IIA, IIIA, IV, terminal). */
export function StatusScreen({ inscripcion, branding, reciénEnviado = false }: { inscripcion: InscripcionPublica; branding: FormBranding; reciénEnviado?: boolean }) {
  ...
  if (reciénEnviado && fase === "IIA_PENDIENTE_FIRMA") {
  ...
  } else if (fase === "IIIA_APROBACION_CUMPLIMIENTO") {
    card = (
      <StatusCard tone="indigo" titulo="En revisión por Cumplimiento" descripcion="Sus documentos fueron revisados y aprobados. El equipo de Cumplimiento está evaluando su inscripción." razonSocial={razonSocial}>
        <div className="rounded-xl border border-indigo-100 bg-indigo-50 px-4 py-3">
          <p className="text-sm leading-relaxed text-indigo-700">Una vez que Cumplimiento apruebe su expediente, Contabilidad completará el registro. Le notificaremos cuando haya novedades.</p>
        </div>
      </StatusCard>
    );
  } else if (fase === "COMPLETADO") {
    card = <StatusCard tone="green" titulo="¡Inscripción Completada!" descripcion="Su empresa ha sido registrada exitosamente como cliente." razonSocial={razonSocial} />;
    ...
    card = (
      <StatusCard tone="orange" titulo="Registro en curso" descripcion="Cumplimiento aprobó su expediente. El equipo de Contabilidad está completando el registro de su empresa." razonSocial={razonSocial}>
