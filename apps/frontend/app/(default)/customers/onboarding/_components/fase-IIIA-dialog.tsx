...
import { CUSTOMER_MODULO, ESTADO_DOC_BADGE, ESTADO_DOC_LABEL, EVALUACION_CONFIG, getOnboardingErrorMessage, InfoItem, RIESGO_BADGE_SOLID } from "./ui-config";

function DocLinkRow({ label, estado, storageId, inscripcionId, onOpen, loading }: { label: string; estado?: string; storageId?: Id<"_storage">; inscripcionId?: Id<"onboardingClientes">; onOpen?: () => void; loading?: boolean }) {
...
/** Fase IIIA — Cumplimiento approval (escalated by evaluation type). */
export default function FaseIIIADialog({ inscripcionId, open, onOpenChange }: { inscripcionId: Id<"onboardingClientes">; open: boolean; onOpenChange: (open: boolean) => void }) {
  const inscripcion = useQuery(api.onboarding.customers.obtenerInscripcionPorId, { inscripcionId });
  const revisiones = useQuery(api.onboarding.customers.obtenerRevisionDocumentos, { inscripcionId });
  const completarFaseIIIA = useMutation(api.onboarding.customers.completarFaseIIIA);
  const rechazarCumplimiento = useMutation(api.onboarding.customers.rechazarCumplimiento);
  ...
  async function handleVerFormato() {
    ...
    try {
      // Short-lived read-only link to the signed form viewer (never rotates the customer's tokens).
      const { url } = await emitirEnlace({ modulo: CUSTOMER_MODULO, inscripcionId, scope: "SIGN", viewOnly: true });
  ...
  async function handleAprobar() {
    setLoading(true);
    try {
      await completarFaseIIIA({ inscripcionId, decision: "APROBADO", observaciones: observacionesAprobacion.trim() || undefined });
      toast.success("Inscripción aprobada por Cumplimiento. Pasa a Fase IV — Creación en Contabilidad.");
      onOpenChange(false);
    } catch (e) {
  ...
  async function handleRechazar() {
    if (!motivoCliente.trim()) {
      toast.error("Ingresa el motivo de rechazo que verá el cliente.");
      return;
    }
    ...
    setLoading(true);
    try {
      await rechazarCumplimiento({ inscripcionId, motivoCliente: motivoCliente.trim(), motivoInterno: motivoInterno.trim() });
      toast.success("Inscripción rechazada por Cumplimiento.");
      onOpenChange(false);
    } catch (e) {
      ...
      <DialogContent className="flex max-h-[90vh] max-w-xl flex-col gap-0 p-0">
        <DialogHeader className="border-b border-slate-100 px-6 py-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle className="text-base font-semibold text-slate-900">Fase IIIA — Aprobación Cumplimiento</DialogTitle>
        ...
        <div className="flex-1 space-y-5 overflow-y-auto px-6 py-5">
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <InfoItem label="Tipo de evaluación">
                <Badge variant="outline" className={cn("font-semibold", RIESGO_BADGE_SOLID[evaluacion])}>
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-1.5">
              <DocLinkRow label="Formulario de inscripción (firmado)" onOpen={handleVerFormato} loading={abriendoFormato} />
              {revisiones?.map((doc) => (
          ...
          {!showRechazo ? (
            <div className="space-y-3">
              <div className="rounded-xl border border-indigo-100 bg-indigo-50/60 p-4">
                <p className="text-sm leading-relaxed text-indigo-700">
                  Todos los documentos han sido revisados. Verifica el nivel de riesgo y la información del cliente antes de tomar una decisión.
                </p>
              </div>
              <div className="space-y-1.5">
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1.5">
                <Label className="text-xs font-semibold uppercase tracking-wide text-slate-500" htmlFor="motivoCliente">
                  Motivo para el cliente
                  <span className="ml-1 text-[10px] font-normal normal-case text-slate-400">(visible externamente)</span>
                </Label>
                <Textarea id="motivoCliente" value={motivoCliente} onChange={(e) => setMotivoCliente(e.target.value)} placeholder="Explica al cliente el motivo del rechazo..." rows={3} className="resize-none border-slate-200 bg-slate-50" />
              </div>
              <div className="space-y-1.5">
