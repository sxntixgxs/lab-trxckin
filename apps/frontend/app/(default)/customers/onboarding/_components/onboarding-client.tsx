...
import ConfigRoles from "@/app/(default)/suppliers/onboarding/_components/config-roles";
...
import { CUSTOMER_MODULO } from "./ui-config";
...
export default function OnboardingClient() {
  ...
  const seguimiento = useQuery(api.onboarding.customers.obtenerInscripcionesConUltimaFase, {
  ...
  const tareas = useQuery(api.onboarding.customers.obtenerMisTareas, {});
  ...
  const tabsVisibles = esSoloResponsable || esSoloConsulta || esConsultaCreacion ? TABS.filter((t) => t.id === "seguimiento") : TABS;
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-4 py-6 lg:px-6">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="text-xl font-semibold tracking-tight text-slate-900 sm:text-2xl">Inscripción de Clientes</h1>
          <p className="mt-1 text-sm text-slate-500">Gestión del ciclo completo de inscripción y actualización de clientes.</p>
        </div>
          ...
          {esFull && (
            <>
              <Button
                ...
                onClick={() => downloadExcel(filteredRows, `inscripciones-clientes-filtradas-${Date.now()}.xlsx`)}
              >
              ...
              <Button
                ...
                onClick={() => downloadExcel(allRows, `inscripciones-clientes-todas-${Date.now()}.xlsx`)}
              >
                <FileSpreadsheet className="mr-2 h-4 w-4 text-green-600" />
                Exportar todo
              </Button>
      ...
      <div className="mb-6">
        ...
        <div className="mt-6">
          ...
          {tabActual === "reportes" && esFull && <TablaReporteProcesos />}
          {tabActual === "configuracion" && <ConfigRoles modulo={CUSTOMER_MODULO} canEdit={seguimiento?.access.isAdmin === true} />}
        </div>
      </div>
    </div>
  );
