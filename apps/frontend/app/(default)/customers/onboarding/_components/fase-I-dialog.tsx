...
import { getOnboardingErrorMessage, InfoItem, RIESGO_BADGE_SOLID, TipoSolicitudChip } from "./ui-config";
...
function StorageLink({ storageId, label, inscripcionId }: { storageId: Id<"_storage"> | undefined; label: string; inscripcionId: Id<"onboardingClientes"> }) {
  ...
  if (storageId) return <span className="text-sm text-slate-400">Cargando...</span>;
  return <>—</>;
}

/**
 * Fase I — risk analysis. Only reachable after a phase return: the customer process starts in
 * Fase II with the risk computed by the responsable. Lets Cumplimiento adjust the matrix and re-approve.
 */
export default function FaseIDialog({ inscripcionId, open, onOpenChange }: { inscripcionId: Id<"onboardingClientes">; open: boolean; onOpenChange: (open: boolean) => void }) {
  const inscripcion = useQuery(api.onboarding.customers.obtenerInscripcionPorId, { inscripcionId });
  const completarFaseI = useMutation(api.onboarding.customers.completarFaseI);
  const actualizarCamposFaseI = useMutation(api.onboarding.customers.actualizarCamposFaseI);
  ...
  useEffect(() => {
    ...
    setEdit({
      representanteLegalNombre: dg.representanteLegalNombre ?? "",
      email: dg.email ?? "",
      celular: dg.celular ?? "",
      codigoCiiu: a2?.codigoCiiu ?? "",
      actividadEconomica: a2?.actividadEconomica ?? "",
      codigoCiiuSecundario: a2?.codigoCiiuSecundario ?? "",
      actividadEconomicaSecundaria: a2?.actividadEconomicaSecundaria ?? "",
  ...
  const tipoActual = inscripcion.tipoEvaluacion ?? "INDEFINIDO";
  const previewRisk = editMode ? computeCustomerRisk(edit) : null;
  const actSecVista = a2?.actividadEconomicaSecundaria?.trim() || (a2?.codigoCiiuSecundario?.trim() ? (CIIU_ACTIVIDAD[a2.codigoCiiuSecundario.trim()] ?? "") : "");
  ...
  async function handleCompletar() {
    ...
    try {
      ...
      toast.success("Fase I completada. Se envió la invitación al formulario al cliente.");
      onOpenChange(false);
    } catch (e) {
  ...
  async function handleGuardarEdicion() {
    ...
    try {
      const result = await actualizarCamposFaseI({
        inscripcionId,
        representanteLegalNombre: edit.representanteLegalNombre || undefined,
        email: edit.email || undefined,
        celular: edit.celular || undefined,
        codigoCiiu: edit.codigoCiiu,
        actividadEconomica: edit.actividadEconomica || (CIIU_ACTIVIDAD[edit.codigoCiiu.trim()] ?? ""),
        codigoCiiuSecundario: edit.codigoCiiuSecundario,
        actividadEconomicaSecundaria: edit.actividadEconomicaSecundaria || (CIIU_ACTIVIDAD[edit.codigoCiiuSecundario.trim()] ?? ""),
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-2xl gap-0 overflow-y-auto p-0">
        <DialogHeader className="border-b border-slate-100 px-6 py-5 pr-14 sm:pr-16">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="mt-0.5 text-xs text-slate-500">
                ...
                {dg.tipoPersona ? ` · Persona ${TIPO_PERSONA_LABELS[dg.tipoPersona]?.toLowerCase() ?? ""}` : ""}
              </p>
            </div>
            {!editMode ? (
          ...
          <div className="mt-3">
            <TipoSolicitudChip tipoSolicitud={dg.tipoSolicitud ?? "INSCRIPCIÓN"} className="text-[11px]" />
          </div>
        </DialogHeader>

        <div className="space-y-6 px-6 py-5">
          <div className="space-y-3">
            <p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Contacto del cliente</p>
            <div className="grid grid-cols-2 gap-2">
              {editMode ? (
                <>
                  <EditItem label="Representante legal">
                    <Input value={edit.representanteLegalNombre} onChange={(e) => setEdit((s) => ({ ...s, representanteLegalNombre: e.target.value }))} className={inputCls} />
                  </EditItem>
                  <EditItem label="Email">
                    <Input type="email" value={edit.email} onChange={(e) => setEdit((s) => ({ ...s, email: e.target.value }))} className={inputCls} />
                  </EditItem>
                  <EditItem label="Celular">
                    <Input value={edit.celular} onChange={(e) => setEdit((s) => ({ ...s, celular: e.target.value }))} className={inputCls} />
                  </EditItem>
                </>
              ) : (
                <>
                  <InfoItem label="Representante legal">{dg.representanteLegalNombre || "—"}</InfoItem>
                  <InfoItem label="Email">{dg.email || "—"}</InfoItem>
                  <InfoItem label="Celular">{dg.celular || "—"}</InfoItem>
                  <InfoItem label="Email representante legal">{dg.representanteLegalEmail || "—"}</InfoItem>
                </>
              )}
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              {editMode ? (
                <>
                  <EditItem label="Código CIIU principal">
                    <Input value={edit.codigoCiiu} onChange={(e) => setEdit((s) => ({ ...s, codigoCiiu: e.target.value }))} className={inputCls} />
                    {edit.codigoCiiu.trim() && CIIU_ACTIVIDAD[edit.codigoCiiu.trim()] ? <p className="mt-1 text-[11px] leading-snug text-slate-500">{CIIU_ACTIVIDAD[edit.codigoCiiu.trim()]}</p> : null}
                  </EditItem>
                  <EditItem label="Actividad económica principal">
                    <Input value={edit.actividadEconomica} onChange={(e) => setEdit((s) => ({ ...s, actividadEconomica: e.target.value }))} placeholder="Se toma del catálogo si se deja vacío" className={inputCls} />
                  </EditItem>
                  <EditItem label="Código CIIU secundario">
                    <Input value={edit.codigoCiiuSecundario} onChange={(e) => setEdit((s) => ({ ...s, codigoCiiuSecundario: e.target.value }))} placeholder="Opcional" className={inputCls} />
                  </EditItem>
                  <EditItem label="Actividad económica secundaria">
                    <Input value={edit.actividadEconomicaSecundaria} onChange={(e) => setEdit((s) => ({ ...s, actividadEconomicaSecundaria: e.target.value }))} placeholder="Opcional" className={inputCls} />
                  </EditItem>
                  <EditItem label="Sector económico">
                    <Select value={edit.sectorEconomico} onValueChange={(v) => setEdit((s) => ({ ...s, sectorEconomico: v }))}>
                      ...
                      <SelectContent>
                        {CUSTOMER_SECTOR_OPTIONS.map((opt) => (
                  ...
                  <EditItem label="Monto anual">
                    <Select value={edit.montoAnual} onValueChange={(v) => setEdit((s) => ({ ...s, montoAnual: v }))}>
                      ...
                      <SelectContent>
                        {CUSTOMER_MONTO_OPTIONS.map((opt) => (
                          <SelectItem key={opt} value={opt}>
                            {opt}
                          </SelectItem>
                        ))}
                  ...
                  <EditItem label="Jurisdicción nacional">
                    <Select value={edit.jurisdiccionNacional} onValueChange={(v) => setEdit((s) => ({ ...s, jurisdiccionNacional: v, jurisdiccionInternacional: "" }))} disabled={!!edit.jurisdiccionInternacional}>
                      <SelectTrigger className={inputCls}>
                        <SelectValue placeholder="Seleccionar" />
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  <EditItem label="Jurisdicción internacional">
                    <Select value={edit.jurisdiccionInternacional} onValueChange={(v) => setEdit((s) => ({ ...s, jurisdiccionInternacional: v, jurisdiccionNacional: "" }))} disabled={!!edit.jurisdiccionNacional}>
                      <SelectTrigger className={inputCls}>
                        <SelectValue placeholder="Seleccionar" />
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  <EditItem label="Listas restrictivas">
                    <Select value={edit.listas === "SÍ" ? "SÍ" : "NO"} onValueChange={(v) => setEdit((s) => ({ ...s, listas: v }))}>
                      <SelectTrigger className={inputCls}>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                ...
                <>
                  <InfoItem label="CIIU principal">{a2?.codigoCiiu || "—"}</InfoItem>
                  <InfoItem label="Actividad económica">{a2?.actividadEconomica || (a2?.codigoCiiu ? CIIU_ACTIVIDAD[a2.codigoCiiu] : "") || "—"}</InfoItem>
                  <InfoItem label="CIIU secundario">{a2?.codigoCiiuSecundario?.trim() || "—"}</InfoItem>
                  ...
                  {inscripcion.condicionesPago_12 && (
                    <>
                      <InfoItem label="Forma de pago">{inscripcion.condicionesPago_12.formaPago}</InfoItem>
                      <InfoItem label="Plazo">{inscripcion.condicionesPago_12.plazo}</InfoItem>
                    </>
                  )}
                </>
              )}
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <Input id="listas" value={listas} onChange={(e) => setListas(e.target.value)} placeholder={matriz.listas || "Ej: Sin coincidencias en listas restrictivas"} className="border-slate-200 bg-slate-50" />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <Textarea id="observaciones" value={observaciones} onChange={(e) => setObservaciones(e.target.value)} placeholder="Observaciones sobre el análisis de riesgo..." rows={3} className="resize-none border-slate-200 bg-slate-50" />
            </div>
          </div>
        </div>
