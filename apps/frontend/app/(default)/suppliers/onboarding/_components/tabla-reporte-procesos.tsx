...
async function exportExcel(rows: ReporteRow[]) {
  ...
  const crearResumen = (tipo: TipoConteoDias) => {
    const headers = ["Empresa", "Tipo Solicitud", "Tipo Documento", "Nro. Documento", "Razón Social", "Riesgo", "Estado Final", "Fecha Inicio Proceso", "Fecha Cierre", `${getDiasLabel(tipo)} totales`];
    ...
    for (const r of rows) {
      for (const f of r.fases) {
        ...
        data.push([
          ...
          r.riesgo,
          r.estadoFinal,
          SUPPLIER_FASE_LABELS[f.fase] ?? f.fase,
          f.estado,
  ...
  XLSX.writeFile(wb, `reporte-proveedores-${Date.now()}.xlsx`);
}
...
/** Time-per-phase report of finished processes, with calendar/business-day toggle and 4-sheet Excel. */
export default function TablaReporteProcesos() {
  const { empresaActiva } = useEmpresaFilter();
  const data = useQuery(api.onboarding.suppliers.obtenerReporteProcesos, { empresa: empresaActiva ?? undefined }) as ReporteRow[] | undefined;
                        ...
                        <td colSpan={9} className="px-3 py-3">
                          <div className="overflow-hidden rounded-lg border border-slate-200 bg-white">
                            <table className="w-full text-xs">
                              ...
                              <tbody className="divide-y divide-slate-100">
                                {[...r.fases]
                                  .sort((a, b) => SUPPLIER_FASES_ORDEN_REPORTE.indexOf(a.fase) - SUPPLIER_FASES_ORDEN_REPORTE.indexOf(b.fase))
                                  .map((f, i) => {
                                    const d = calcularDias(f.fechaInicio, f.fechaCompletado, tipoConteoDias);
                                    return (
                                      <tr key={`${r.inscripcionId}-${f.fase}-${i}`}>
                                        <td className="px-3 py-2 text-slate-800">{SUPPLIER_FASE_LABELS[f.fase] ?? f.fase}</td>
