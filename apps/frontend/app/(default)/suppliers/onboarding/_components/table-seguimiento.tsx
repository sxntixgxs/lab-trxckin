...
import { CorreoHistorialPanel, CorreoStatusBadge, pickCorreoResumenVisible, renderFaseIHistorialDetalle } from "./correo-status";
...
export default function TableSeguimiento({ data, onDataChange }: { data: SeguimientoData | undefined; onDataChange?: (all: ExportRow[], filtered: ExportRow[]) => void }) {
  ...
  const puedeGestionarCorreo =
    !!access && !modoSoloConsulta && (access.isAdmin || access.roles.length > 0 || detailInscripcion?.matriz_00.responsableId === access.usuarioId);
