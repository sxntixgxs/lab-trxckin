...
type Tarea = Doc<"onboardingProveedoresFases"> & { inscripcion: Doc<"onboardingProveedores"> };

export default function MisTareas({ tareas }: { tareas: Tarea[] | undefined }) {
  const [dialog, setDialog] = useState<FaseDialogKey | null>(null);
  const [selectedId, setSelectedId] = useState<Id<"onboardingProveedores"> | null>(null);

  function abrirGestion(fase: string, inscripcionId: Id<"onboardingProveedores">) {
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-xs">
        <header className="flex items-center gap-3 border-b border-slate-200 px-6 py-5">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Inscripciones asignadas a ti que requieren acción.</p>
          </div>
          {!isLoading && (
